...
function replace(animals, name, replacement) {
    for (var index = 0; index <= animals.length - 1; index++) {
        var nameAnimal = animals[index].name; 
        if (name === nameAnimal) {
...
function add(animals, animal) {
 if(animal.name.length >= 1 && animal.species.length >= 1 &&  !search(animals, animal.name)) {
 //  if(search(animals, animal.name) === undefined && search(animals, animal.name) === undefined) {
            animals.push(animal);
    }
  } 
